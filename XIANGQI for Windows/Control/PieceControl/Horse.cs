...
namespace Control
{
    class Horse
    {
        public bool Ma(int X, int Y, int chozenX, int chozenY, Chess[,] Matrix)
        {
            ...
            {
                if (Matrix[chozenX, chozenY + 2 * (Y - chozenY) / Math.Abs(Y - chozenY)].side != Chess.Player.god)
                {
                    return false;
                }
            }
            ...
            {
                if (Matrix[chozenX + (X - chozenX) / Math.Abs(X - chozenX) * 2, chozenY].side != Chess.Player.god)
                {
                    return false;
                }
            }
